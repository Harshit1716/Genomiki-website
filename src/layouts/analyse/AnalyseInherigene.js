...
import { rows } from "./data/inherigence";
...
function AnalyticsPage() {
  ...
  const classificationData = useMemo(() => {
    ...
    sampleData.forEach((item) => {
      const key = item["Genomiki Variant Classification"] || "Unknown";
      map[key] = (map[key] || 0) + 1;
    });
  ...
  const zygosityData = useMemo(() => {
    const map = {};
    sampleData.forEach((item) => {
      const key = item.Zygosity || "Unknown";
      map[key] = (map[key] || 0) + 1;
    });
          ...
          <MDBox p={3}>
            <Grid container spacing={3}>
              ...
              <Grid item xs={12}>
                <Card>
                  <MDBox p={2}>
                    <MDTypography variant="h6">Depth Distribution</MDTypography>
                    <ResponsiveContainer width="100%" height={300}>
